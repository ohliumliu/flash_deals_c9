...
if __name__ == "__main__":
    ...
    # lines = ssc.textFileStream('history').map(parse)
    ...
    def process(time, rdd):
        ...
        try:
            ...
            wordCountsDataFrame = \
                spark.sql("select title, count(*) as total from words group by title")
            wordCountsDataFrame.show()
            
            # load model pipeline
            model = PipelineModel.load('kmeans')
            model.transform(wordsDataFrame).select("6_kmeans").show(5)
            wordsDataFrame.show()
        except:
            pass
